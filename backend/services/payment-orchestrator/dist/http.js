...
async function createHttpServer() {
    ...
    app.get("/internal/dashboard/payments", async (request) => {
    ...
    app.get("/internal/dashboard/events", async (request) => {
        const q = request.query || {};
        const limit = Math.min(Number(q.limit) || 20, 100);
        const { rows } = await (0, db_1.query)(`SELECT id, event_type, payload, source, created_at
       FROM payment_events
       ORDER BY created_at DESC
       LIMIT $1`, [limit]);
        const items = rows.map((row) => {
            ...
            try {
                if (row.event_type === "PaymentInitiated") {
                    description = `Payment ${p.paymentId ?? ""} initiated by ${p.payer ?? "?"} → ${p.payee ?? "?"} for ${p.amount ?? "?"}.`;
                }
    ...
    app.get("/internal/dashboard/reconciliation", async (request, reply) => {
        const q = request.query || {};
